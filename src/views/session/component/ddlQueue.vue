<template>
  <section class="ddl-queue">
    <div class="ddl-queue__head">
      <h3 class="ddl-queue__title">{{ $t('session.DDL Queue') }}</h3>
      <span class="ddl-queue__count">{{ total }}</span>
    </div>

    <vxe-table
      class="ddl-queue__table"
      :data="pageData"
      :border="false"
      align="center"
      resizable
...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      const { data: { entity } } = await SessionApi.ddl_queue(id);
    ...
    async killSession(row) {
      const id = this.$route.params.id;
      await SessionApi.kill(id, row.host, row.queryId, 'queue');
